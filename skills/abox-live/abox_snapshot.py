...
def _first_user_text(o):
    """First real user prompt text — the /resume fallback when no AI title exists.
    Skips slash-command envelopes and system/tool-result tags (they start with '<')."""
    m = o.get("message")
...
def parse_turns(path, max_turns):
    """Returns (turns, title). `title` mirrors what `/resume` shows: the session's
    AI-generated title (`ai-title`/`aiTitle`), else a `summary` record, else the first
    real user message."""
    out = []
    ai_title = ""      # newest wins — the title is rewritten as the session evolves
    summary = ""
    first_user = ""
    try:
...
def build(show_all, max_turns, recent_min):
    ...
    data = []
    for r in rows:
        turns, title = parse_turns(r["path"], max_turns) if r["path"] else ([], "")
        data.append({
        ...
        for root, label in all_roots():
            for f in glob.glob(os.path.join(root, "*", "*.jsonl")):
                ...
                if mt < cut:
                    continue
                idle = time.time() - mt
                turns, title = parse_turns(f, max_turns)
                data.append({
                    ...
                    "pid": "", "live": False, "state": "ended", "title": title,
                    "turns": turns,
                })
...
def shorten(s, n):
    s = " ".join(str(s or "").split())   # collapse newlines/runs so it fits one cell
    ...
    for s in data:
        ...
        p.append('<tr><td><span class="dot %s"></span></td><td>%s</td>'
                 ...
                 % (s["state"], esc(s["container"]), esc(s["session"][:8]),
                    ...
                    len(s["turns"]), esc(s["evidence"]), esc(s["cwd"])))
    p.append("</table>")
